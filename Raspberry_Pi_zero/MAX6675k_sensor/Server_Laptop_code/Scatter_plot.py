...
def change_time_format(raw):
    # first creaing a new dataframe object

  #  Convert the timestamp column to datetime
    raw['timestamp'] = pd.to_datetime(raw['timestamp'])
    # Extract the minute and second from the timestamp column
    raw['minute_second'] = raw['timestamp'].dt.strftime('%M:%S')
    return raw
...
def plotting_function():
    ...
    # moreover, the presence of this line eliminates the need to use the command plt.show()
    ...
    new_data_frame = change_time_format(data_frame)

    try:
        plt.scatter(new_data_frame['minute_second'],
                    data_frame['temperature'], marker="o")
        ...
        plt.xlabel("Timestamp in minute:seconds", color='red', fontdict={
            'fontsize': 18, 'fontstyle': 'italic'})
        plt.ylabel("Temperature reading", color='blue', fontdict={
            'fontsize': 18, 'fontstyle': 'italic'})
        # removed the rotation = 45 parameter/attribute
        plt.xticks(new_data_frame['minute_second'], rotation=45)
...
def read_table():
    ...
    df = pd.read_sql(sql_query, conn).tail(40)
